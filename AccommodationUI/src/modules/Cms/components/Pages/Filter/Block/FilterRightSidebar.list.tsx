...
const FilterLeftSidebar: FC<any> = (props) => {
    ...
    const { listData, loading, categoryIds } = props;
    ...
    return (
        <>
            <Col xs={24} md={17} className="right-part-serachfilter">
                ...
                {/* start: package */}
                {loading && <div className="text-center"><Spin
                    className='d-inline-block' /></div>}
                {listData?.map((item, index) => {
                    return (
                        <>
                            <div
                                className="p-3 mb-3 bg-white rounded-2 accommodation-package-box">
                                <Row gutter={[16, 16]}>
                                    <Col xs={24} md={7}>
                                        <div className='accommodation-gallery'>
                                            <img
                                                src={item?.cover_image ? CONSTANT_CONFIG.CDN_URL + 'image/' + item?.cover_image : "https://i.postimg.cc/8z6rkwRJ/room.jpg"} alt={"picture"}
                                            />
                                        </div>
                                    </Col>
                                    <Col xs={24} md={17}>
                                        <div className='accommodation-package'>
                                                ...
                                                <div>
                                                    ...
                                                    <div
                                                        ...
                                                        {item.accommodation_type ?
                                                            <CreditCardOutlined /> : null} {lang == "en" ? item.accommodation_type_name_en : item.accommodation_type_name_bn}
                                                    </div>
                                                </div>
                                                <div
                                            ...
                                            <div
                                                ...
                                                <div className='text-end see-availability fw-bold'>
                                                    <Link
                                                        to={getLangPath(lang, '/building-details/' + item.id + "/" + item.accommodation_type)}
                                                    >
