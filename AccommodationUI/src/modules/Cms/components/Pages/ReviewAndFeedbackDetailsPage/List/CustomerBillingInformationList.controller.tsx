...
import BookingPageListFilter from "./CustomerBillingInformationList.filter";
import BookingPageListing from "./CustomerBillingInformationList.listing";
import BookingPageListPagination from "./CustomerBillingInformationList.pagination";
...
const CustomerBillingInformationListController: FC = () => {
    ...
    const handleDelete = (value: any, action: any): Promise<any> => {
        return new Promise((resolve, reject) => {
            if (value && action === 'ok') {
                setLoading(true);
                FackApi.delete(value)
                    .then(res => {
                        Message.success(t('Delete bookingPage successfully.'));
                        handleReloadListing();
                        setLoading(false);
                        resolve(res);
                    })
    ...
    const handleBulkActions = (action: string) => {
        ...
        if (action === 'active') {
            ...
            AntModal.confirm(t('Delete BookingPage'), t('Are you sure you wish to delete selected bookingPages?'), selectedRowKeys, confirmBulkDelete, 'Delete');
        }
    };
    ...
    const handleActions = (action: string, recordId?: any) => {
        ...
        if (action === 'delete') {
            AntModal.confirm(t('Delete BookingPage'), t('Are you sure you want to delete this bookingPage?'), recordId, handleDelete, 'Delete');
        }
    };
    ...
    return (
        <>
            ...
            <Form form={formRef} name="bookingPageListingFilterForm" initialValues={initialValues} >
                <BookingPageListFilter
                    filters={filters}
...
export default CustomerBillingInformationListController;
