...
import CategoriesListFilter from "./CategoriesList.filter";
import CategoriesListing from "./CategoriesList.listing";
import CategoriesListPagination from "./CategoriesList.pagination";
...
const CategoriesListController: FC = () => {
  ...
  const queryState = {
    ...
    pagination: {
      currentCategories:
        queryParams?.page || initialState.pagination.currentCategories,
  ...
  const loadData = (): Promise<any> => {
    return new Promise((resolve, reject) => {
      setLoading(true);
      CategoriesApi.list(payload.current)
        .then((res) => {
  ...
  const handleUrl = (): void => {
    ...
    if (
      pagination.currentCategories !== initialState.pagination.currentCategories
    ) {
      urlObject.page = pagination.currentCategories;
    }
  ...
  const handlePayload = (): void => {
    payload.current = {
      ...
      $skip: pagination.pageSize * (pagination.currentCategories - 1),
    };
  };
  ...
  const handleReset = () => {
    ...
    setPagination({
      currentCategories: initialState.pagination.currentCategories,
