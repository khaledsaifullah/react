...
import ServicesListFilter from "./ServicesList.filter";
import ServicesListing from "./ServicesList.listing";
import ServicesListPagination from "./ServicesList.pagination";
...
import "./ServicesList.style.scss";
...
const ServicesListController: FC = () => {
  ...
  const queryState = {
    ...
    pagination: {
      currentServices:
        queryParams?.page || initialState.pagination.currentServices,
  ...
  const loadData = (): Promise<any> => {
    return new Promise((resolve, reject) => {
      setLoading(true);
      ServicesApi.list(payload.current)
        .then((res) => {
  ...
  const handleUrl = (): void => {
    ...
    if (
      pagination.currentServices !== initialState.pagination.currentServices
    ) {
      urlObject.page = pagination.currentServices;
    }
  ...
  const handlePayload = (): void => {
    payload.current = {
      ...
      $skip: pagination.pageSize * (pagination.currentServices - 1),
    };
  };
  ...
  const handleReset = () => {
    ...
    setPagination({
      currentServices: initialState.pagination.currentServices,
  ...
  const handleOnChanged = (fieldName: string, value: any, text?: any) => {
    ...
    else if(fieldName === 'organogram_id') {
      setFilters({
          ...filters,
          organogram_id: value,
      });
