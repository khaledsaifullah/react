...
import BlocksListFilter from "./BlocksList.filter";
import BlocksListing from "./BlocksList.listing";
import BlocksListPagination from "./BlocksList.pagination";
...
const BlocksListController: FC = () => {
  ...
  const loadData = (): Promise<any> => {
    return new Promise((resolve, reject) => {
      setLoading(true);
      BlocksApi.list(payload.current)
        .then((res) => {
  ...
  const handleDelete = (value: any, action: any): Promise<any> => {
    return new Promise((resolve, reject) => {
      if (value && action === "ok") {
        setLoading(true);
        BlocksApi.delete(value)
          .then((res) => {
            Message.success(t("Delete blocks successfully."));
            handleHideView();
            handleReloadListing();
            setLoading(false);
            resolve(res);
  ...
  const handleBulkActions = (action: string) => {
    ...
    if (action === "active") {
      ...
      AntModal.confirm(
        t("Delete Blocks"),
        t("Are you sure you wish to delete selected blocks?"),
        selectedRowKeys,
        confirmBulkDelete,
        "Delete"
      );
  ...
  const handleActions = (action: string, recordId?: any) => {
    ...
    if (action === "view") {
      ...
      AntModal.confirm(
        t("Delete Blocks"),
        t("Are you sure you want to delete this blocks?"),
        recordId,
        handleDelete,
        "Delete"
      );
  ...
  return (
    <div className="listing-page-container listing-page-container-blocks">
      <Form
        form={formRef}
        name="blocksListingFilterForm"
...
export default BlocksListController;
